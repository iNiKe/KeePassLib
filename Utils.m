...
+ (ByteBuffer *)createByteBufferForString:(NSString *)string coding:(NSStringEncoding)encoding
{
	...
	NSUInteger size = [string lengthOfBytesUsingEncoding:encoding];
	ByteBuffer *result = [[ByteBuffer alloc] initWithSize:size];
	NSRange range;
	range.location = 0; range.length=[string length];
	[string getBytes:result._bytes maxLength:size usedLength:nil encoding:encoding options:0 range:range remainingRange:nil];
	return result;
}
